...
namespace XmlRpc
{
    ...
    public class XmlRpcClient //: IDisposable
    {
        ...
        {
            [DebuggerStepThrough] get { return this.getcontentlength(instance); }
        }
		/*
        ...
        {
            return handleEvent((XmlRpcDispatch.EventType)eventType);
        }
 ...
 */
		...
		{	
			if (_connectionState & ConnectionState.NO_CONNECTION)
				if (! setupConnection())
					return false;
			return _connectionState & ConnectionState.NO_CONNECTION;	
		}
...
bool execute(string method, XmlRpcValue parameters, XmlRpcValue result)
{
  ...
  if (_connectionState != ConnectionState.IDLE || ! parseResponse(out result))
    return false;
...
bool executeCheckDone(XmlRpcValue result)
{
  ...
  return true;
		...
		// Possible IO states for the connection
			enum ConnectionState { NO_CONNECTION, CONNECTING, WRITE_REQUEST, READ_HEADER, READ_RESPONSE, IDLE };
...
bool generateRequest(string methodName, XmlRpcValue parameters)
{
  ...
  {
    body += PARAMS_TAG;
    if (parameters.getType() == XmlRpc.ValueType.TypeArray)
    {
